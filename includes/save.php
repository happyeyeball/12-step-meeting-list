...
function tsml_save_post($post_id, $post, $update) {
	...
	if (empty($_POST['group'])) {
		...
		/* ADDED JDF 2019-12-18 */
		//meeting language
		if (!$update || strcmp($old_meeting->primary_language, $_POST['primary_language']) !== 0) {
			$changes[] = 'primary_language';
			if (empty($_POST['primary_language'])) {
				delete_post_meta($post->ID, 'primary_language');
			} else {
				update_post_meta($post->ID, 'primary_language', sanitize_text_field($_POST['primary_language']));
			}
		}
	...
	if (count($tsml_notification_addresses) && count($changes)) {
		...
		$fields = array('name', 'day', 'time', 'end_time', 'types', 'notes', 'location', 
			...
			'website', 'primary_language', 'email', 'phone', 'mailing_address', 'venmo',
